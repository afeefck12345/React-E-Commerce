...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Handle both shapes: array directly OR { products: [...] }
        const data = res.data;
  ...
  const filteredProducts = products
    .filter((p) => {
      ...
      // ✅ Fix 2: case-insensitive category comparison
      const matchCategory =
        category === "All" ||
  ...
  // ✅ Fix 3: use _id instead of id (MongoDB uses _id)
  ...
  const paginated = uniqueProducts.slice(
